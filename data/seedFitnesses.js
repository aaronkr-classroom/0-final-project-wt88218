"use strict";

/**
 * Listing 15.9 (p. 224)
 */
const mongoose = require("mongoose"),
  Fitness = require("../models/Fitness"); // 'Course'를 'Fitness'로 수정

// 데이터베이스 연결 설정
mongoose.connect("mongodb://127.0.0.1:27017/ut-nodejs", {
  useNewUrlParser: true,
  useUnifiedTopology: true, // 이 옵션을 추가하여 연결을 안정화
});

const db = mongoose.connection;

// 데이터베이스 연결 에러 처리
db.on('error', console.error.bind(console, 'connection error:'));
db.once('open', function() {
  console.log("Connected to the database");

  var fitnesses = [
    {
      _id: new mongoose.Types.ObjectId(), // ObjectId로 변환
      title: "Node.js 101",
      description: "웹 개발로 알아보는 백엔드 자바스크립트의 이해",
      price: 20000,
      courseImg: "",
    },
    {
      _id: new mongoose.Types.ObjectId(), // ObjectId로 변환
      title: "HTML, CSS, JS 101",
      description: "웹 개발의 시작은 여기서부터",
      price: 10000,
      courseImg: "",
    },
    {
      _id: new mongoose.Types.ObjectId(), // ObjectId로 변환
      title: "Python 101",
      description: "파이썬 기초 문법을 배워봅시다",
      price: 10000,
      courseImg: "",
    },
    {
      _id: new mongoose.Types.ObjectId(), // ObjectId로 변환
      title: "AI + ML 101",
      description: "인공지능과 머신러닝의 기초를 배워봅시다",
      price: 30000,
      courseImg: "",
    },
    {
      _id: new mongoose.Types.ObjectId(), // ObjectId로 변환
      title: "React 101",
      description: "리액트를 배워봅시다",
      price: 15000,
      courseImg: "",
    },
  ];

  var commands = [];

  // 1. Delete all previous data. / 이전 데이터 모두 삭제
  // 2. Set a timeout to allow the database to be cleared. / 데이터베이스가 지워지는 것을 기다리기 위해 타임아웃 설정
  // 3. Create a promise for each courses object. / 코스 객체마다 프라미스 생성.
  // 4. Use Promise.all() to wait for all promises to resolve. / 모든 프라미스가 해결될 때까지 기다리기 위해 Promise.all() 사용.
  // 5. Close the connection to the database. / 데이터베이스 연결 닫기.

  Fitness.deleteMany({})
    .exec()
    .then((result) => {
      console.log(`Deleted ${result.deletedCount} fitness records!`);
      
      fitnesses.forEach((c) => {
        commands.push(
          Fitness.create({
            _id: c._id,
            title: c.title,
            description: c.description,
            price: c.price,
            courseImg: c.courseImg,
          }).then((Fitness) => {
            console.log(`Created Fitness: ${Fitness.title}`);
          })
        );
      });

      console.log(`${commands.length} commands created!`);

      return Promise.all(commands);
    })
    .then((r) => {
      console.log(JSON.stringify(r));
      mongoose.connection.close();
      console.log("Connection closed!");
    })
    .catch((error) => {
      console.log(`Error: ${error}`);
      mongoose.connection.close();
    });
});
